...
exp <- unique(vesk$iso)
ax <- data.frame(iso =setdiff(b32b, exp), "combo"= NA)
vesk2 <- full_join(vesk, ax, by=c("iso"))
vesk2<- vesk2[,1:2]
...
final <- inner_join(world, vesk3, by="iso")
...
base + geom_polygon(data = final, aes(fill= combo)) + 
